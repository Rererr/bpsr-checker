...
- **DPS / Healing / History tabs** — Encounters are saved automatically when combat ends, so you can review past logs.
- **Per-skill breakdown** — Click a player to see damage, hit count, and crit rate for each of that player's skills.
- **Battle Imagine name** — Shows each player's Battle Imagine used in combat (Tina / Airona / Tatta / Basilisk, etc.) appended to the name column of the DPS list (e.g. `Sora-Tina/Airona`). It detects not only your own but also the Imagines used by players around you, including ones that deal no damage (such as revives). The `{imagine}` token in the name-column template lets you reposition it, or remove it to hide. It is always shown in the header of the per-skill breakdown you open by clicking a player.
- **3-minute measurement mode** — A dedicated mode for training dummies and boss practice that aggregates for a fixed duration from the start of combat (default 180s, adjustable). The results screen shows per-character and per-skill DPS trend graphs (with time and DPS axes) and a pie chart of the skill breakdown (TOP 10 + others).
...
- Every release is scanned on VirusTotal: [installer](https://www.virustotal.com/gui/file/e4d709e5ae40b34683c59c073365d7afd770e7ff0bf4913379503d17d37d025f/detection) · [portable](https://www.virustotal.com/gui/file/af64cfe36d1218c4e54befcbd95db242e13026258ac932ffee2c9457605adbd9/detection).
