...
- **バトルイマジン名の表示** — 各プレイヤーが装備しているバトルイマジン（ティナ / アルーナ / タータ / バジリスク 等）をレベル（凸数）付きで DPS 一覧の名前列末尾に表示（例: `ソラ-ティナ(3)/アルーナ(1)`）。自分はマップ移動ごと、周囲のプレイヤーはダンジョン読込やボス部屋切替のタイミングで検知し、発動が控えめで見えにくいイマジンも漏れなく表示できる。ロールスキル枠に装備した簡易版バトルイマジン（最大4件）は実装備の2枠とは区別し、`(R:名前)` の形式で追記表示する（例: `ソラ-ティナ(3)/アルーナ(1) (R:サンダーオーガ/ファルファラ)`）。名前列テンプレートの `{imagine}` トークンで表示位置を調整でき、不要なら削除して非表示にできる。プレイヤーをクリックして開くスキル別内訳の見出しでは常に表示。イマジン名は設定から開く専用の編集ウィンドウで手動メンテでき、表示名の上書きや特定イマジンの非表示を指定できる（上書きは表示のみに適用され、検知や記録には影響しない）
- **計測モード** — 模擬戦やボス練習向けに、戦闘開始から固定時間で集計する専用モード (デフォルト 180 秒、変更可)。計測後の結果画面では、キャラ別・スキル別の DPS 推移グラフ（時間軸・DPS 目盛り付き）と、スキル内訳の円グラフ（TOP10＋その他）を確認可能。**自己ベストを計測時間ごとに自動記録**し、更新時は結果画面に「新記録！」を表示。TOP3 は金/銀/銅で強調され、日時入りの結果画像をワンクリックでコピーして Discord 等へ共有できる
- **イマジン (バトルイマジン) デバフタイマー** — ティナ / アルーナ / タータ / バジリスク / ガトグリフの免疫デバフ残時間を独立オーバーレイで表示。既定では DPS 一覧と同期し、表示する顔ぶれ・並び順を自動追従する（並び順の追従は個別に OFF 可）。ピンアイコンで各プレイヤーをタイマーから隠す/表示を切替。同期を OFF にすればピンで手動追加した相手だけを表示する従来運用に切替でき、一括クリアの導線も用意。表示するイマジン種類はパーティ構成に合わせて個別に選択可。行を詰める密表示にも対応。カラムヘッダーにカタカナ名・キャラ別識別色（ティナ赤 / アルーナ緑 / タータ紫 / バジリスク茶 / ガトグリフ水色）付きリングで視認性を確保
...
- **デバフタイマー専用モード (軽量)** — DPS/回復の集計を停止してデバフタイマーのみ動作させる省リソースモード。有効中は DPS 一覧に「イマジン専用モード中です」と集計を止めている旨を表示し、その場のボタンから設定パネルを開かずに解除できる（数値が出ない状態を不具合と誤解しないため）
- **職業アイコン表示** — DPS 一覧の名前の先頭に職業アイコンをロール色（アタッカー / タンク / ヒーラー）で表示（既定 ON）。名前列テンプレートから `{classIcon}` トークンを外せば非表示にできる
- **行バーの表示方式を選択可** — DPS 一覧の各行の横バー（ゲージ）の基準を 4 種から選択: トップ比（既定）/ 全体比（パーティ合計に対するシェア）/ 固定基準（右端をユーザー指定の値に固定し、直近 N 秒間の平均 DPS をバーの長さで表示）/ 自分基準（自分のバーを 50% 位置に固定し、他プレイヤーを自分との相対比率で表示。50% ガイド線付き）。バーの濃さも「なし / 控えめ / くっきり」の 3 段階から選べ、バー幅の変化を滑らかにするアニメーションも有効化できる（CPU 使用率がやや上がるため既定 OFF）
- **2 列コンパクト表示** — DPS テーブルを 2 列に分割して横幅を節約するレイアウトモード
- **DPS 推移グラフ** — 各プレイヤー行の DPS 推移を小グラフで可視化
- **常に最前面表示・クリックスルー・タスクバー/トレイ切替** — オーバーレイ運用に必須。タイトルバーのボタンから最小化・最前面ピン止めをワンタッチ切替でき、ウィンドウ幅が狭いときはヘッダーが自動でアイコン表示に切り替わる。最小化は既定で**タスクトレイへ格納**（メインを最小化するとオーバーレイも一緒に退避し、トレイアイコンの左クリックまたは右クリック「メインを表示」で復帰）。設定の「タスクバーに常駐」をオンにすると、メイン・各オーバーレイが**個別のタスクバーボタン**になり、各ウィンドウの最小化ボタンでタスクバーへ最小化できる。各オーバーレイは右上の × ボタンで閉じられる（設定の表示トグルと連動）。クリックスルー（背後のゲームへクリックを素通し）はタスクトレイから切替・解除。各ウィンドウは縁ドラッグでリサイズ可能
- **グローバルショートカットキー** — 計測の初期化・一時停止/再開・3分計測の開始/キャンセル・一覧コピー・常に最前面切替の各操作に、ゲームにフォーカスがある状態でも使えるショートカットキーを割り当て可能。設定パネルから登録でき、既定はすべて未設定
...
- **フッターのお問い合わせ導線** — メイン画面下部の半透明フッター（設定で非表示可）から、開発者へのお問い合わせフォームや GitHub Issue への報告を既定ブラウザで開ける
...
- すべてのリリースは VirusTotal でスキャンしています（最新リリースの結果: [インストーラ](https://www.virustotal.com/gui/file/ba9f5f81c7ce1d7f5e20146df2785f6b2da21c538f387f37309fa7f6d52c3f2c/detection) ・ [ポータブル](https://www.virustotal.com/gui/file/ea6cf347129a8218586da77a9abf57a6e0e1875073ba9edf870f907fef7b3daa/detection)）。
