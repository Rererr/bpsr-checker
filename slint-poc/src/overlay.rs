//! ウィンドウ周りのネイティブ操作（winit へ降りる）。
//! クリックスルー切替・モニタ列挙を提供する。

use i_slint_backend_winit::WinitWindowAccessor;

/// 物理座標系でのモニタ矩形（左上原点＋サイズ＋スケール）。
#[derive(Debug, Clone)]
...
/// （winit の cursor-hittest は false で「当たり判定なし＝素通し」）
pub fn set_click_through(window: &slint::Window, enabled: bool) {
    window.with_winit_window(|w| {
        if let Err(e) = w.set_cursor_hittest(!enabled) {
            eprintln!("[overlay] set_cursor_hittest failed: {e}");
        }
    });
}

/// no-frame 窓のドラッグ移動を開始する（マウス押下イベント中に呼ぶこと）。
/// winit が OS の移動ループへ処理を引き渡す。
pub fn start_drag(window: &slint::Window) {
    window.with_winit_window(|w| {
        if let Err(e) = w.drag_window() {
            eprintln!("[overlay] drag_window failed: {e}");
        }
    });
}
