...
- ウィンドウ単位の撮影は暗背景の上で行うと透明部分が締まる。Win11 では非 TopMost の
  フルスクリーン背景が topmost 窓を覆うことがあるため、背景を TopMost にして撮影対象窓を
  SetWindowPos(HWND_TOPMOST) で再アサートする。リサイズ直後は再描画前で透明になるので
  2 秒以上待ってからキャプチャする。
